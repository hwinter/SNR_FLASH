...
!!*********************************************************************
subroutine read_clouds(N, x_pos, y_pos, z_pos, cloud_rad,cloud_rho)
    implicit none

    integer :: N, iii
    real :: x_pos(N), y_pos(N), z_pos(N), cloud_rad(N), cloud_rho(N)
    ...
    DO iii=1, N
       !! PRINT *, iii
       read(2,*) x_pos(iii), y_pos(iii), z_pos(iii), cloud_rad(iii), cloud_rho(iii)
    ...
    !! DO iii=1, N
       !! PRINT *, x_pos(iii), y_pos(iii), z_pos(iii), cloud_rad(iii), cloud_rho(iii)
    !! ENDDO
    return
...
integer function get_number_of_clouds()
  ...
  real, allocatable :: x_pos(:), y_pos(:), z_pos(:), cloud_rad(:), cloud_rho(:)
  logical :: file_exists
  ...
  if (file_exists) then
	...
	call system("pwd")
	call system("ls /pool/cluster3/hwinter/programs/Flashcode/FLASH4/object/clouds.txt")
...
real function test_current_position(x,y,z,N, x_pos, y_pos, z_pos, cloud_rad, cloud_rho)
   integer :: iii,N
   real ::x,y,z,x_pos(N), y_pos(N), z_pos(N), cloud_rad(N), cloud_rho(N), distance

   test_current_position=0.0
   !print*,'z',z	
   
   do iii=1, N
      distance=sqrt(((x-x_pos(iii))**2) + ((y-y_pos(iii))**2) + ((z-z_pos(iii))**2) )
      !! print *, distance, x, x_pos(iii), y, y_pos(iii), cloud_rad(iii), cloud_rho(iii)
...
!! Variables for clouds
  ...
  real :: new_rho,rho_out, distance, test_current_position
!! Variables needed to read the cloud data and perform the test.

  integer :: NN, get_number_of_clouds, test
...
!!Read in the file and fill in the values.
  call read_clouds(NN, x_pos, y_pos, z_pos, cloud_rad, cloud_rho)
